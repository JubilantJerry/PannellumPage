    ...
    function Ba(a) {
      ...
      f.addEventListener("loadend", function () {
        ...
        temp_image.onload = function () {
          var webgl = document.createElement('canvas').getContext("experimental-webgl", { alpha: !1, depth: !1 })
          var max_size = webgl.getParameter(webgl.MAX_TEXTURE_SIZE);
          var found_size = Math.max(temp_image.width / 2, temp_image.height);
          if (found_size > max_size) {
            var canvas2 = document.createElement('canvas');
            var ctx2 = canvas2.getContext('2d');
            canvas2.width = Math.floor(temp_image.width * max_size / found_size);
            canvas2.height = Math.floor(temp_image.height * max_size / found_size);
            ctx2.drawImage(temp_image, 0, 0, canvas2.width, canvas2.height);
            canvas2.toBlob((blob) => {
              P.src = E.URL.createObjectURL(blob);
            }, 'image/jpeg');
          } else {
            P.src = temp_image.src;
          }
        };
        temp_image.src = E.URL.createObjectURL(a);
      });
